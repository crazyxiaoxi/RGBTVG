...
from .vl_transformer_mdetr import build_vl_transformer
...
class DynamicMDETR(nn.Module):
    ...
    def feautures_sampling(self, sampling_query, reference_point, feature_map, pos, stage):
        ...
        if self.args.modality == 'rgbt':
            feature_map = feature_map.reshape(bs, channel, self.visual_feature_map_h, self.visual_feature_map_w*2) # (bs, channel, h, w)
    ...
    def forward(self, img_data, text_data):
        ...
        if self.args.modality == 'rgbt':
            ...
            rgb_mask , rgb_src = self.visumodel(rgb_data)
            thermal_mask , thermal_src = self.visumodel(thermal_img)
            rgb_src = self.visu_proj(rgb_src)  # (H*W, B, channel)
