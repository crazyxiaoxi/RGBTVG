...
class Modified_CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        cls_pos = self.position_embedding.weight[0:1, :] #  1  * 768
...
class Cross_Attention_Fusion(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.head_dim * self.num_heads != self.projection_dim :
            raise ValueError(
                f"embed_dim must be divisible by num_heads (got `embed_dim`: {self.projection_dim } and `num_heads`:"
                f" {self.num_heads})."
            )
        self.scale = self.head_dim**-0.5
        self.dropout = config.attention_dropout

        self.k_proj = nn.Linear(self.projection_dim, self.projection_dim)    
        self.v_proj = nn.Linear(self.projection_dim, self.projection_dim) 
        self.q_proj = nn.Linear(self.projection_dim, self.projection_dim) 
        self.out_proj = nn.Linear(self.projection_dim, self.projection_dim) 
    ...
    def forward(
        ...
        bsz, tgt_len, embed_dim = hidden_states_ir.size()

        query_states = self.q_proj(hidden_states_ir) * self.scale
        query_states = self._shape(query_states, -1, bsz)  #torch.Size([32, 8, 197, 64])

        key_states = self._shape(self.k_proj(hidden_states_rgb), -1, bsz) #torch.Size([77, 32, 768])->torch.Size([32, 12, 77, 64])
        # todo检查self._shape
        value_states = self._shape(self.v_proj(hidden_states_rgb), -1, bsz)
...
class MMVG(nn.Module):
    def __init__(self, args):
        ...
        if (args.model == "ViT-L/14-336"):
            ...
            # ../dataset_and_pretrain_model/pretrain_model/pretrained_weights/CLIP/clip_b_ml_cascade_maskrcnn_model_224.pth
            """
        ...
        if args.modality == 'rgbt':
            self.clip.vision_model_ir = CLIP_Vision_Model_with_Crossmodal_Bridge(args, self.clip.vision_model,
                                                                          self.adapt_layer, self.extract_text_layer,
                                                                          self.clip.text_model.config)
        """
        ...
        if args.modality == 'rgbt':
            self.visu_proj_ir = nn.Linear(self.hidden_dim, self.hidden_dim)
            # self.visu_fuse = nn.Linear(self.hidden_dim*2, self.hidden_dim)
            self.visu_fuse = Cross_Attention_Fusion(self.clip.vision_model.config)
        ...
        if args.modality=='rgbt':
            ...
            self.ml_visual_projection_ir = nn.Linear(len(self.extract_vision_layer) * self.clip.vision_model.config.hidden_size,
                                              self.hidden_dim)
            self.ml_visual_projection_ir.weight = nn.Parameter(torch.cat([self.clip.visual_projection.weight.clone() for i
                                                                in range(len(self.extract_vision_layer))], dim=1))
    ...
    def set_HiLoRA(self, args):
        ...
        if open_lora:
            ...
            # peft_config = AdaLoraConfig(target_modules=target_modules, inference_mode=False, r=32, lora_alpha=64,
                                    #  lora_dropout=0.1, bias='none')
    ...
    def forward(self, img_data, text_data):
        ...
        if self.args.modality =="rgbt":
            clip_image_features_ir = self.clip.vision_model_ir(self.adapt_layer, ml_text_features, reg_src, image_tensors_ir,
                                                    output_attentions=True, output_hidden_states=True,
                                                    return_dict=True)  # B * 197 * 512
            ...
            img_cls_embed_ir = self.clip.visual_projection(clip_image_features_ir["pooler_output"])  # torch.Size([64, 512]) 不影响推理，返回值
            ...
            image_features_ir = self.ml_visual_projection_ir(ml_image_features_ir)
        # import pdb
        # pdb.set_trace()

        if self.args.modality =="rgbt":
            visu_src = self.visu_proj(image_features.float())  # (N*B)xC
            visu_src_ir = self.visu_proj_ir(image_features_ir.float())
            # visu_src = self.visu_fuse(torch.cat([visu_src,visu_src_ir],dim=-1))
            visu_src,_ = self.visu_fuse(visu_src_ir,visu_src)
        else:
            visu_src = self.visu_proj(image_features.float())  # (N*B)xC
...
# def interpolate_vl_pos_embed(model: MMVG, checkpoint_path: str, original_imsize: int = 224):
#     """
#     Loads a checkpoint, interpolates the visual part of vl_pos_embed,
#     and loads the rest of the state_dict into the model.

#     Args:
#         model (MMVG): The current MMVG model instance (already initialized with target imsize).
#         checkpoint_path (str): Path to the MMVG checkpoint file.
#         original_imsize (int): The image size used when the checkpoint was trained.
#     """
#     print(f"Loading checkpoint from: {checkpoint_path}")
#     checkpoint = torch.load(checkpoint_path, map_location='cpu')
#     state_dict = checkpoint['model_state_dict'] if 'model_state_dict' in checkpoint else checkpoint

#     # 1. Extract the original vl_pos_embed.weight from the checkpoint
#     if 'vl_pos_embed.weight' not in state_dict:
#         print("Warning: 'vl_pos_embed.weight' not found in checkpoint. Skipping interpolation.")
#         model.load_state_dict(state_dict, strict=False)
#         return

#     checkpoint_vl_pos_embed_weight = state_dict['vl_pos_embed.weight']
#     hidden_dim = model.hidden_dim # Should be consistent with the checkpoint

#     # 2. Determine source (checkpoint) and target (current model) parameters
#     # Source (checkpoint) dimensions
#     source_patch_size = model.patch_size # Assuming patch_size is consistent (e.g., 16)
#     source_num_visu_token = int((original_imsize / source_patch_size) ** 2) # e.g., (224/16)^2 = 196
#     source_num_text_token = model.num_text_token # Assuming max_query_len is consistent (e.g., 77)

#     # Target (current model) dimensions
#     target_imsize = model.imsize # e.g., 640
#     target_patch_size = model.patch_size # e.g., 16
#     target_num_visu_token = int((target_imsize / target_patch_size) ** 2) # e.g., (640/16)^2 = 1600
#     target_num_text_token = model.num_text_token # e.g., 77

#     # Assertions for sanity checks
#     assert source_patch_size == target_patch_size, "Patch size must be consistent between source and target models."
#     assert hidden_dim == checkpoint_vl_pos_embed_weight.shape[1], "Hidden dimension mismatch."
#     # Ensure text token count is consistent, as we won't interpolate it
#     # Note: The +1 for text_eos_token is handled by the slicing logic if it's always the last token.
#     # If text_pos_embed_loaded includes the EOS token, its size should match target_num_text_token + 1
#     # For CLIP, max_query_len is typically 77, which includes the EOS token.
#     expected_source_text_len = checkpoint_vl_pos_embed_weight.shape[0] - 1 - source_num_visu_token
#     expected_target_text_len = model.vl_pos_embed.weight.shape[0] - 1 - target_num_visu_token
#     assert expected_source_text_len == expected_target_text_len, \
#         f"Text token length mismatch. Source: {expected_source_text_len}, Target: {expected_target_text_len}"


#     print(f"Interpolating vl_pos_embed from {original_imsize}x{original_imsize} ({source_num_visu_token} visu tokens) "
#           f"to {target_imsize}x{target_imsize} ({target_num_visu_token} visu tokens).")

#     # 3. Decompose the checkpoint vl_pos_embed_weight
#     # REG token is always at index 0
#     reg_pos_embed_loaded = checkpoint_vl_pos_embed_weight[0:1, :] # (1, hidden_dim)

#     # Visual tokens are from index 1 to 1 + source_num_visu_token
#     visu_pos_embed_loaded = checkpoint_vl_pos_embed_weight[1 : 1 + source_num_visu_token, :] # (source_num_visu_token, hidden_dim)

#     # Text tokens are after visual tokens, up to the end
#     text_pos_embed_loaded = checkpoint_vl_pos_embed_weight[1 + source_num_visu_token : , :] # (num_text_token + 1, hidden_dim)

#     # 4. Interpolate the visual positional embedding
#     source_visu_grid_size = int(math.sqrt(source_num_visu_token)) # e.g., 14
#     target_visu_grid_size = int(math.sqrt(target_num_visu_token)) # e.g., 40

#     # Reshape for 2D interpolation: (1, hidden_dim, H, W)
#     # Permute to (hidden_dim, num_visu_token) then reshape to (hidden_dim, H, W) then unsqueeze(0)
#     visu_pos_embed_loaded_reshaped = visu_pos_embed_loaded.permute(1, 0).reshape(
#         hidden_dim, source_visu_grid_size, source_visu_grid_size
#     ).unsqueeze(0)

#     interpolated_visu_pos_embed = F.interpolate(
#         visu_pos_embed_loaded_reshaped,
#         size=(target_visu_grid_size, target_visu_grid_size),
#         mode="bicubic",
#         antialias=True, # Recommended for high-quality interpolation
#         align_corners=False # Often preferred for positional embeddings
#     )

#     # Reshape back to (target_num_visu_token, hidden_dim)
#     interpolated_visu_pos_embed = interpolated_visu_pos_embed.squeeze(0).permute(1, 2, 0).reshape(
#         target_num_visu_token, hidden_dim
#     )

#     # 5. Re-compose the new vl_pos_embed_weight
#     new_vl_pos_embed_weight = torch.cat([
#         reg_pos_embed_loaded,
#         interpolated_visu_pos_embed,
#         text_pos_embed_loaded # Text part remains unchanged in length
#     ], dim=0)

#     # 6. Assign the new weight to the model's vl_pos_embed
#     model.vl_pos_embed.weight = nn.Parameter(new_vl_pos_embed_weight)
#     print(f"vl_pos_embed.weight interpolated and assigned. New shape: {model.vl_pos_embed.weight.shape}")

#     # 7. Load the rest of the state_dict (excluding vl_pos_embed)
#     # Remove the original vl_pos_embed from the state_dict to avoid strict=False warning/error for it
#     if 'vl_pos_embed.weight' in state_dict:
#         del state_dict['vl_pos_embed.weight']
    
#     # Load the remaining state_dict. Use strict=False in case there are other minor mismatches
#     # or if you intentionally skip other parts.
#     model.load_state_dict(state_dict, strict=False)
#     print("Remaining model state_dict loaded with strict=False.")