...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, args, layers, output_dim, heads, width=64):
        super().__init__()
...
class VisionTransformer(nn.Module):
    def __init__(self, args, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        super().__init__()
        input_resolution =  args.imsize
        self.input_resolution = input_resolution
        self.args = args
        ...
        self.positional_embedding = nn.Parameter(scale * torch.randn(197, width))
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.flatten(2).transpose(1, 2)  # B L H
        ...
        abs_pos = self.positional_embedding[1:, :]  # 196 * 768
        ...
        if size != h or size != w:
            ...
            x = x + position_embedding.repeat(batch_size, 1, 1)
        else:  # 14 == 14
