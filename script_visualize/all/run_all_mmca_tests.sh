#!/bin/bash
# ===================== MMCA Comprehensive Test Script =====================
# 3 datasets × 3 modalities = 9 test combinations
# Usage: bash visualize_scripts/all/run_all_mmca_tests.sh

echo "Starting MMCA comprehensive test..."
echo "Test scope:"
...
# Model base path
MODEL_BASE_PATH="../dataset_and_pretrain_model/result/MMCA"
...
for dataset in "${DATASETS[@]}"; do
    for modality in "${MODALITIES[@]}"; do
        ...
        MODEL_CHECKPOINT="${MODEL_BASE_PATH}/MMCA_224_${modality}_${DATASET_SHORT}_best.pth"

        echo ""
        echo "Test $CURRENT_TEST/$TOTAL_TESTS: $dataset + $modality"
        echo "   Model: $MODEL_CHECKPOINT"
        echo "   Output: ./visual_result/mmca/${dataset}/${modality}"
        echo "----------------------------------------"
        ...
        # Run test
        if bash visualize_scripts/shell_scripts/visualize_mmca.sh "$dataset" "$modality" "$MODEL_CHECKPOINT"; then
...
echo ""
echo "MMCA comprehensive test completed!"
echo "========================================"
...
if [ $SUCCESS_COUNT -eq $TOTAL_TESTS ]; then
    echo ""
    echo "All MMCA tests completed successfully!"
    exit 0
else
    echo ""
    echo "Some MMCA tests failed, please check the failed list above"
    exit 1
fi
