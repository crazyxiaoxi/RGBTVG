#!/bin/bash
# ===================== OneRef Base Comprehensive Test Script =====================
# 3 datasets × 3 modalities = 9 test combinations
# Usage: bash visualize_scripts/all/run_all_oneref_tests.sh

echo "Starting OneRef_B comprehensive test..."
echo "Test scope:"
...
# Model base path
MODEL_BASE_PATH="../dataset_and_pretrain_model/result/OneRef_B"
...
for dataset in "${DATASETS[@]}"; do
    for modality in "${MODALITIES[@]}"; do
        ...
        MODEL_CHECKPOINT="${MODEL_BASE_PATH}/ONEREF_base_rec_${modality}_${DATASET_SHORT}_best.pth"

        echo ""
        echo "Test $CURRENT_TEST/$TOTAL_TESTS: $dataset + $modality"
        echo "   Model: $MODEL_CHECKPOINT"
        echo "   Output: ./visual_result/oneref_base/$dataset/$modality"
        echo "----------------------------------------"
        ...
        # Run test
        if bash visualize_scripts/shell_scripts/visualize_oneref_base.sh "$dataset" "$modality" "$MODEL_CHECKPOINT"; then
...
echo ""
echo "OneRef_B comprehensive test completed!"
echo "========================================"
...
if [ $SUCCESS_COUNT -eq $TOTAL_TESTS ]; then
    echo ""
    echo "All OneRef_B tests completed successfully!"
    exit 0
else
    echo ""
    echo "Some OneRef_B tests failed, please check the failed list above"
    exit 1
fi
