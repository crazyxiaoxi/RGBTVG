...
# 使用示例：bash visualize_scripts/all/run_all_oneref_tests.sh

echo "🚀 开始 OneRef_B 全面测试..."
echo "测试范围："
...
for dataset in "${DATASETS[@]}"; do
    for modality in "${MODALITIES[@]}"; do
        ...
        echo "   输出: ./visual_result/oneref_base/$dataset/$modality"
        echo "----------------------------------------"
...
if [ $SUCCESS_COUNT -eq $TOTAL_TESTS ]; then
    echo ""
    echo "🎊 所有 OneRef_B 测试都成功完成！"
    exit 0
else
    echo ""
    echo "⚠️  部分 OneRef_B 测试失败，请检查上述失败列表"
    exit 1
fi
