...
# 使用示例：bash visualize_scripts/all/run_all_hivg_large_tests.sh
...
for dataset in "${DATASETS[@]}"; do
    for modality in "${MODALITIES[@]}"; do
        ...
        echo "   输出: ./visual_result/hivg_large/$dataset/$modality"
        echo "----------------------------------------"
...
if [ $SUCCESS_COUNT -eq $TOTAL_TESTS ]; then
    echo ""
    echo "🎊 所有 HiVG_L 测试都成功完成！"
    exit 0
else
    echo ""
    echo "⚠️  部分 HiVG_L 测试失败，请检查上述失败列表"
    exit 1
fi
